...
# Usage:
#   .\compile.ps1              # build once -> test\gameserver_yyyy-MM-dd_HHmm.exe
#   .\compile.ps1 -Release     # build with -ldflags "-s -w"
#   .\compile.ps1 -Watch       # watch .go files and rebuild on change
#   .\compile.ps1 -Clean       # remove test\gameserver_*.exe
#   .\compile.ps1 -Clean -Release  # clean then release build
...
} elseif (-not $Watch -and ($Release -or -not $Clean)) {
    Build
}
